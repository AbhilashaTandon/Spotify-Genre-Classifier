...
class PlaylistData:
    def __init__(self, json_obj):
        ...
        for playlist_id in json_obj:
            ...
            self.artist_occurences_per_playlist.update(current_playlist_artists)

        self.unique_artists = [
            x[0] for x in self.artist_occurences_per_playlist.most_common(NUM_ARTISTS)
        ]
        ...
        self.artist_to_idx = {
            artist: idx for idx, artist in enumerate(self.unique_artists)
        }
    ...
    def get_most_similar(self, artists, n):
        ...
        most_sim = [(idx, sim) for idx, sim in sim_artists if idx not in artist_idxs]
...
def main():
    with open("data/output.json", mode="r") as data:
        ...
        print(
            "| Most Similar Artists to " + ", ".join(artists) + " | Cosine Similarity |"
        )
        ...
        reduced = np.array(
            umap.UMAP(
                n_neighbors=100, n_components=10, n_jobs=-1, metric="cosine", min_dist=0
            ).fit_transform(tf_idf.T)
        )

        labels_numeric = list(
            SpectralClustering(n_clusters=NUM_GENRES, random_state=5).fit_predict(
                reduced
            )
        )
        ...
        genre_artist_freq = [
            Counter() for _ in range(np.min(labels_numeric), np.max(labels_numeric) + 1)
        ]  # dict of artists partitioned by genre and their popularity
        for artist, label in zip(plist_data.unique_artists, labels_numeric):
            genre_artist_freq[label][artist] = (
                plist_data.artist_occurences_per_playlist[artist]
            )
        ...
        for idx, counter in enumerate(genre_artist_freq):
            print(
                genres[idx]
                + ": "
                + ", ".join([x[0] for x in counter.most_common(NUM_GENRES)])
            )
            print()

        reducer = umap.UMAP(
            n_neighbors=50, n_components=2, n_jobs=-1, metric="cosine", min_dist=0
        )
        ...
        df = pd.DataFrame(
            {
                "x": umap_embedded[:, 0],
                "y": umap_embedded[:, 1],
                "names": plist_data.unique_artists,
                "labels": named_genres,
            }
        )
